...
def uncollected_cleanup_view(request):
    ...
    #응답 보낼 데이터
    data = uncollected_data.values('id', 'coast_name', 'latitude', 'longitude', 'litter_bags_count','calculated_litter_amount', 'main_litter_type', 'completion_photo_collection_site')
...
def mark_as_collected(request, cleanup_id):
    try:
        cleanup = Cleanup.objects.get(id=cleanup_id)
